...
ax1.set_ylabel(r'$ V_{w} [\mathrm{m/min}]$' , fontsize=18)
...
ax1.set_xlabel(r'$ V_{w} [\mathrm{m/min}]$' , fontsize=18)
...
#Liquid property
mu=10.8       #Viscosity [mPas]
sigma=67.64   #Surface tension [mN/m]
theta_d=60    #DCA degree
theta_s=60    #SCA degree

# Operabiltiy window
# c=1.0   # cos term
c=np.cos(theta_d/180*np.pi)+np.cos(theta_s/180*np.pi)   #Radian으로 변환; cos term 주로 1.0 이라고 두어왔음.
H0=200  # Uniform die lip configuration in MCPL
Ld=800.0  #Ld=Lu
...
#c=0.2   # cos term
c=np.cos(theta_d/180*np.pi)+np.cos(theta_s/180*np.pi)   #Radian으로 변환; cos term 주로 1.0 이라고 두어왔음.
H0=75  # Uniform die lip configuration in MCPL
Ld=800.0  #Ld=Lu
